...
void insert_info(FILE *f,struct student_info S)
{
    ...
    scanf("%s %d %s %s %s",S.std_name,&S.id,S.classE,S.city,S.DOB);
...
void display(FILE *f,struct student_info S)
{
    ...
    while(fread(&S,sizeof(S),1,f)==1)
    {
        ...
        printf("\nstd_name=%s,std_id=%d,class=%s,city=%s,DOB=%s",S.std_name,S.id,S.classE,S.city,S.DOB);
    }
}
...
void modify_class(FILE *f,struct student_info S)
{
 char modify_class[20];
    ...
    long L=sizeof(S);
            ...
            while(fread(&S,sizeof(S),1,f)==1)
            {
                if(strcmpi(S.classE,modify_class)==0)
                {
...
void sort(FILE *f,struct student_info S)
{
    ...
    for(i=0;i<c;i++)
       printf("\nstd_name=%s,std_id=%d,class=%s,city=%s, DOB=%s",T[i].std_name,T[i].id,T[i].classE,T[i].city,T[i].DOB);
}
